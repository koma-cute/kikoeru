 ...
 * @param {ReadableStream} readerStream Image data stream.
 * @param {String} rjcode Work RJ code (only the 6 digits, zero-padded).
 */
const saveCoverImageToDisk = (readerStream, rjcode) => new Promise((resolve, reject) => {
  // TODO: don't assume image is a jpg?
  var writerStream = fs.createWriteStream(path.join(config.rootDir, 'Images', `RJ${rjcode}.jpg`));
